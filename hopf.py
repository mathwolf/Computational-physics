...
def hopfRK(s, t, param):
    # Returns right hand side of Hopf ODEs
    # Inputs
    #   s           state vector [x, y]
    #   t           time
    #   param       parameter vector [a]
    # Output
    ...
    # Compute derivatives
    x_prime = a*x + y - x * (x**2+y**2)
    y_prime = -x + a*y - y * (x**2+y**2)
...
def main():
    # Compute trajectories for the Hopf equations using
    # an adaptive Runge-Kutta routine

    # Set initial state of system
    s = input("Enter the initial x value: ")
    x = float(s)
    s = input("Enter the initial y value: ")
    y = float(s)
    state = [x, y]

    # Set the parameters 
    s = input("Enter the parameter value (a): ")
    a = float(s)
    param = [a]
    ...
    for i in range(nstep):
        ...
        state, time, tau = rka(state, time, tau, err, hopfRK, param)
    ...
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title('Hopf model phase plot')
